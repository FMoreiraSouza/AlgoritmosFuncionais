...
pegarCodigo indice armarios = 
    case Map.lookup indice armarios of
        Nothing -> Left $ "Armario nao existe"
        Just (estado, codigo) -> Right codigo
...
aluguel :: Int -> Armarios -> Either String Armarios -- nao usei pegarCodigo
aluguel indice armarios = 
    case Map.lookup indice armarios of
        Nothing -> Left $ "Armario nao existe"
        Just (estado, codigo) ->
            if estado == Livre
                then Right $ (insert indice (Alugado,codigo) armarios)
                else Left $ "Armario esta ocupado"
...
devolucao indice codigo armarios =
    case (pegarArmario indice armarios) of
        Right (estado, c) -> if c == codigo
                                then if estado == Alugado
                                        then Right $ (insert indice (Livre,codigo) armarios)
                                        else Left "Armario esta livre"
                                else Left "Codigo incorreto"
        _ -> Left "Armario nao existe"
